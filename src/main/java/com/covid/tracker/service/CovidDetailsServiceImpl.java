...
public class CovidDetailsServiceImpl implements CovidDetailsService {
    ...
    public CovidDetailsServiceImpl() {

    }

    public CovidDetailsServiceImpl(CountryRepository countryRepository, ApiHistoryRepository apiHistoryRepository, CovidRestRepository covidRestRepository, CovidDataRepository covidDataRepository) {
        this.countryRepository = countryRepository;
        this.apiHistoryRepository = apiHistoryRepository;
        this.covidRestRepository = covidRestRepository;
    ...
    public List<Country> getCountries(boolean fetchDb) {
        ...
        try {
            ...
            if (isOutdated) {
				...
				//countriesInDB = countryRepository.findAll(); // redundant as we came because no 'country' entry in db exist
                ...
                isOutdated = false; // why we are setting it to false, its not in loop or no place where it is rechecked
            ...
            throw new CovidRapidAPIException(HttpStatus.INTERNAL_SERVER_ERROR, "Exception occurred while getting data from Rapid API", e.getDetailedMessage());
        } catch (Exception e) {
            throw new CovidException(String.format("Exception occurred while updating get countries"), e);
        }
